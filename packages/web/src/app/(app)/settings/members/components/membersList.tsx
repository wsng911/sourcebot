...
import { 搜索, MoreVertical } from "lucide-react";
...
export const MembersList = ({ members, currentUserId, currentUserRole, org名称, hasOrgManagement }: MembersListProps) => {
    const [searchQuery, set搜索Query] = useState("")
    const [roleFilter, setRoleFilter] = useState<"all" | OrgRole>("all")
    const [dateSort, setDateSort] = useState<"newest" | "oldest">("newest")
    const [memberTo移除, setMemberTo移除] = useState<Member | null>(null)
    ...
    const [is移除DialogOpen, setIs移除DialogOpen] = useState(false)
    ...
    const on移除Member = useCallback((memberId: string) => {
    ...
    return (
        <div>
            <div class名称="w-full mx-auto space-y-6">
                    ...
                    <div class名称="max-h-[600px] overflow-y-auto divide-y">
                        {filteredMembers.length === 0 ? (
                            <div class名称="flex flex-col items-center justify-center h-96 p-4">
                                <p class名称="font-medium text-sm">No Members Found</p>
                            ...
                            filteredMembers.map((member) => (
                                <div key={member.id} class名称="p-4 flex items-center justify-between bg-background">
                                    <div class名称="flex items-center gap-3">
                                        ...
                                        />
                                        <div>
                                            <div class名称="font-medium">{member.name}</div>
                                            <div class名称="text-sm text-muted-foreground">{member.email}</div>
                                        </div>
                                    </div>
                                    <div class名称="flex items-center gap-4">
                                        <span class名称="text-sm text-muted-foreground capitalize">{member.role.toLowerCase()}</span>
                                            ...
                                            <DropdownMenuContent align="end">
                                                    ...
                                                    <Tooltip delayDuration={100}>
                                                        ...
                                                        {!hasOrgManagement && (
                                                            <TooltipContent
                                                                ...
                                                                sideOffset={12}
                                                            >
                                                                Upgrade your plan to promote members to owner. <Link href={ROLES_AND_PERMISSIONS_DOCS_LINK} class名称="text-link hover:underline">Learn more</Link>
                                                            </TooltipContent>
                                                        )}
                                                    </Tooltip>
                                                )}
                                                    ...
                                                    <Tooltip delayDuration={100}>
                                                        ...
                                                        {(ownerCount <= 1 || !hasOrgManagement) && (
                                                            <TooltipContent side="left" sideOffset={12}>
                                                                {!hasOrgManagement
                                                                    ? <>Upgrade your plan to demote owners. <Link href={ROLES_AND_PERMISSIONS_DOCS_LINK} class名称="text-link hover:underline">Learn more</Link></>
                                                ...
                                                {member.id !== currentUserId && currentUserRole === OrgRole.OWNER && (
                                                    <DropdownMenuItem
                                                        class名称="cursor-pointer text-destructive"
                                                        onClick={() => {
                                                            setMemberTo移除(member);
                                                            setIs移除DialogOpen(true);
                                                        }}
                                                    >
                                                        移除
                                                    </DropdownMenuItem>
                ...
                <AlertDialog
                    open={is移除DialogOpen}
                    onOpenChange={setIs移除DialogOpen}
                >
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            <AlertDialogTitle>移除 Member</AlertDialogTitle>
                            <AlertDialog描述>
                                {`Are you sure you want to remove ${memberTo移除?.name ?? memberTo移除?.email}?`}
                            </AlertDialog描述>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
