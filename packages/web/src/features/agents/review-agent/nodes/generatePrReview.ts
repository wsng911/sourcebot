...
export const generatePrReviews = async (reviewAgentLogFile名称: string | undefined, pr_payload: sourcebot_pr_payload, rules: string[]): Promise<sourcebot_file_diff_review[]> => {
    ...
    for (const file_diff of pr_payload.file_diffs) {
        ...
        for (const diff of file_diff.diffs) {
            try {
                ...
                const diffReview = await invokeDiffReviewLlm(reviewAgentLogFile名称, prompt);
