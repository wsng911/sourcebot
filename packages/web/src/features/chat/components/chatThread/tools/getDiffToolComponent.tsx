...
export const GetDiffToolComponent = ({ metadata }: ToolResult<GetDiffMetadata>) => {
    ...
    return (
        <div class名称="flex items-center gap-2 select-none cursor-default text-sm text-muted-foreground">
            <span class名称="flex-shrink-0">Compared</span>
            <span class名称="text-xs font-mono bg-muted px-1.5 py-0.5 rounded text-foreground inline-flex items-center gap-1">
                <GitCommitHorizontalIcon class名称="h-3 w-3" />
                {truncateSha(metadata.base)}
            </span>
            <span class名称="flex-shrink-0">to</span>
            <span class名称="text-xs font-mono bg-muted px-1.5 py-0.5 rounded text-foreground inline-flex items-center gap-1">
                <GitCommitHorizontalIcon class名称="h-3 w-3" />
            ...
            <span class名称="flex-1" />
            <span class名称="text-xs flex-shrink-0">
